...
enum class RtpEndUserState {
    ...
    ENDING_CALL, // libwebrtc says 'closed' but session-terminate has not gone through
    ENDED, // close UI
    DECLINED_OR_BUSY, // other party declined; no retry button
    CONTACT_OFFLINE, // when `JINGLE_MESSAGE_INIT_STRICT_OFFLINE_CHECK` is true and contact is offline
    ...
    APPLICATION_ERROR, // something rather bad happened; libwebrtc failed or we got an IQ-error

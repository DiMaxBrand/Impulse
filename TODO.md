...
## Message edit: letter-level morph, advanced mode — plan only, blocked on Google Sans Flex, not started

Default mode **shipped in 1.15.0-beta.62** — `ConversationScreen.kt`'s
`computeMorphOps()`/`MorphingMessageText`, wired through `MessageBubble`.
Character-level diff (LCS, per Unicode code point) between the old body and
the new one: removed characters shrink+fade out, inserted characters
grow+fade+blur in, survivors just sit there while `FlowRow` re-measures
every frame and reflows around the animating ones — not a whole-string
crossfade. Unblur and morph run concurrently (not sequenced) so the old
wording is never seen fully sharp at any single frame, preserving the
editing-blur's actual purpose. Scoped to plain text only (no links, `/me`,
emoji-only sizing, or file/media bubbles) and both bodies under 400 code
points; anything outside that still gets the old instant-swap behavior, no
regression. Diff is per-code-point, not grapheme-cluster-aware — a
documented, accepted limitation for heavy emoji use, not attempted.

**What's left — advanced mode**, a new Developer Options / Feature Flags
toggle, off by default, power-user only: instead of every character
animating together, stagger each character's transition slightly in time
(one after another, not synchronized) and have surviving/appearing
characters **stretch** through Google Sans Flex's variable-width/weight axes
as they resize to fill freed or opening space, instead of just translating.
Framed as "feels more premium but takes more time" — deliberately opt-in,
not the default experience.

- **Real blocker**: the stretch half of advanced mode needs Google Sans Flex
  actually integrated first — still a parked, not-started thread (see the
  Typography & UI refresh section above). The staggered-timing half doesn't
  strictly need the font and could ship alone, but that alone is the less
  interesting half of "advanced mode" — probably worth waiting to do both
  together once the font work resumes, rather than shipping stagger-only now
  and stretch later as a second pass.
- **Effort**: medium once Google Sans Flex lands — the diff/reflow engine
  and the FlowRow-based renderer already exist from the default mode; this
  is staggering the per-character timing (each op's `AnimatedVisibility`
  already exists individually, just needs a per-index delay) and swapping
  the width animation for a font-axis animation once there's an actual
  variable font to drive.

...
- [x] Replace the current listening badge with a headphone icon — folded directly into the checkmark's own morph family in `MessageStatusIcon.kt` (new `LISTENING`/`LISTENED`/`LISTEN_UNKNOWN` `CheckmarkPhase` values, `voiceCheckmarkPhase()` override, headband+ear-cup stroke geometry at `CHECK_STROKE_WIDTH` for the in-flight morph only) rather than a separate icon shown alongside it — the double-check morphs on into the headphone in the same slot: dots → check → double-check → headphone. The settled/rest look at either end of any transition is always the real bundled asset — the checkmark's own drawable beforehand, the actual Google Material Symbol headphone (rounded, filled — `ic_headphones_24dp`, same path as the pre-existing `ic_headphones_48dp`) once it arrives — the hand-drawn stroke geometry only ever renders mid-morph. `ListenStatusManager.State` has five values — mapping per state, confirmed:

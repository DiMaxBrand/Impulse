...
- [x] Replace the current listening badge with a headphone icon — folded directly into the checkmark's own morph family in `MessageStatusIcon.kt` (new `LISTENING`/`LISTENED`/`LISTEN_UNKNOWN`/`LISTEN_PAUSED` `CheckmarkPhase` values, `voiceCheckmarkPhase()` override) rather than a separate icon shown alongside it. Went through two approaches for the in-flight morph itself: first a hand-drawn stroke approximation (headband + ear-cup geometry at `CHECK_STROKE_WIDTH`), which never matched the real icon and at one point visibly read as a house mid-transition; **replaced entirely** once it became clear most of the headphone family doesn't need any shape approximation at all — `ic_headphones_24dp` (the exact Google Material Symbols Rounded "headphones" filled path pasted verbatim from `fonts.gstatic.com` inside a `translateY` group, verified byte-for-byte, not the older/numerically-different outline glyph `ic_headphones_48dp` used elsewhere for the audio-file/media-browser icon) is now rendered directly, pixel-exact, for every headphone-family leg *except* the one genuine shape change (checkmark → headphone arrival), which has no path-morph API available and instead crossfades the two real bundled drawables (fade+scale, same convention as `ConversationScreen.kt`'s `messageStatusIconFallback`) rather than approximating an in-between shape by hand. `ListenStatusManager.State` has five values — mapping per state, confirmed:

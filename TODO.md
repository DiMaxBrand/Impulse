...
- [x] Replace the current listening badge with a headphone icon — folded directly into the checkmark's own morph family in `MessageStatusIcon.kt` (new `LISTENING`/`LISTENED`/`LISTEN_UNKNOWN` `CheckmarkPhase` values, `voiceCheckmarkPhase()` override, headband+ear-cup stroke geometry at the same `CHECK_STROKE_WIDTH`) rather than a separate icon shown alongside it — the double-check morphs on into the headphone in the same slot: dots → check → double-check → headphone. `ListenStatusManager.State` has five values — mapping per state, confirmed:
